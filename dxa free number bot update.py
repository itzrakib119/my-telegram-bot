...
# Configuration
# ==========================================
BOT_TOKEN = "8631080640:AAG2tiHif1pr8GnEWVAw5IYsjbv2q0kbNLs"
ADMIN_ID = 8294522264
BASE_URL = f"https://api.telegram.org/bot{BOT_TOKEN}/"
BOT_USERNAME = "RS_NUMBER_PRO19_bot"

# 2oo9 API Configuration
API_2OO9_BASE = "https://api.2oo9.cloud/MXS47FLFX0U/tnevs/@public/api"
API_2OO9_KEY_DEFAULT = "M748FFO2C2I"
...
def auto_cache_menu_thread():
    ...
    while True:
        try:
            ...
            lb_msg = "<blockquote><tg-emoji emoji-id=\"6206096153511990389\">🏆</tg-emoji> <b>𝗧𝗢𝗣 𝟱 𝗟𝗘𝗔𝗗𝗘𝗥𝗕𝗢𝗔𝗥𝗗 (TODAY)</b></blockquote>\n━━━━━━━━━━━━━━━━━\n"
...
# ==========================================
PREMIUM_APPS = {
    "Facebook": {"emoji": "📘", "id": "5296523110734930940"},
    "WhatsApp": {"emoji": "💬", "id": "5296439324512918594"},
    "Telegram": {"emoji": "✈️", "id": "5294193228415801857"},
    "Instagram": {"emoji": "📸", "id": "5296719399330292238"},
    ...
    "Imo": {"emoji": "💭", "id": "5296393407017556498"},
...
def generate_otp_display(service_name, raw_number, message_text):
    ...
    if len(raw_number) > 6:
        masked_number = f"{raw_number[:3]}RS{raw_number[-3:]}"
    else:
    ...
    if main_channel_link:
        row1.append({"text": "𝗖𝗛𝗔𝗡𝗡𝗘𝗟", "url": main_channel_link, "icon_custom_emoji_id": "6221855350593691363", "style": "primary"})
    ...
    markup["inline_keyboard"].append([
        {"text": "GET NUMBER", "url": deep_link, "icon_custom_emoji_id": "5251554614167225549", "style": "danger"}
    ])
...
def get_main_keyboard(user_id):
    keyboard_layout = [
        [
            {"text": "GET NUMBER", "icon_custom_emoji_id": "5251554614167225549", "style": "success"},
            {"text": "LIVE TRAFFIC", "icon_custom_emoji_id": "5429571662737611814", "style": "success"}
        ],
        [
            {"text": "REFER", "icon_custom_emoji_id": "5420145051336485498", "style": "primary"},
            {"text": "PROFILE", "icon_custom_emoji_id": "5334763399299506604", "style": "primary"}
        ],
        [
            {"text": "LEADERBOARD", "icon_custom_emoji_id": "6206096153511990389", "style": "danger"}
        ]
    ]
    if user_id == ADMIN_ID:
    ...
    return {
        "inline_keyboard": [
            [
                {
                    ...
                    "icon_custom_emoji_id": "6203886371363364022", 
                    "style": "danger"
                }
            ],
            [
            ...
            [
                {
                    "text": "RS CONTROL", 
                    "callback_data": "rs_control", 
                    "icon_custom_emoji_id": "6314113614638751405", 
                    "style": "danger"
                }
            ],
            [
...
def rs_control_keyboard():
    grp = bot_settings.get("w_group", "")
    grp_status = grp if grp else "NOT SET"
    return {"inline_keyboard": [
        [{"text": f"MIN WITHDRAW: {bot_settings['min_withdraw']}", "icon_custom_emoji_id": "5352877703043258544", "callback_data": "rs_min_w", "style": "success"},
         {"text": "OTP CONTROL", "icon_custom_emoji_id": "5190576863226933563", "callback_data": "rs_otp_control", "style": "primary"}],
        [{"text": f"REFER REWARD: {bot_settings.get('refer_reward', 0.2)}", "icon_custom_emoji_id": "5420396762189831222", "callback_data": "rs_ref_r", "style": "success"},
         {"text": f"COOLDOWN: {bot_settings['cooldown']}s", "icon_custom_emoji_id": "5337172996211648018", "callback_data": "rs_cool", "style": "primary"}],
        [{"text": f"NUM/REQ: {bot_settings['num_req']}", "icon_custom_emoji_id": "5337132498965010628", "callback_data": "rs_num_req", "style": "success"},
         {"text": "W. METHODS", "icon_custom_emoji_id": "5190899075968441286", "callback_data": "manage_w_methods", "style": "primary"}],
        [{"text": f"W. GROUP: {grp_status}", "icon_custom_emoji_id": "5420517437885943844", "callback_data": "rs_w_group", "style": "success"}],
        [{"text": "BACK", "icon_custom_emoji_id": "5267490665117275176", "callback_data": "back_to_admin", "style": "danger"}]
    ]}

def rs_otp_control_keyboard():
    kb = {"inline_keyboard": [
        [{"text": f"Default Rate: {bot_settings['otp_default_rate']}", "icon_custom_emoji_id": "5352877703043258544", "callback_data": "rs_def_rate", "style": "primary"}],
        [{"text": "Set Service Rate", "icon_custom_emoji_id": "5395444784611480792", "callback_data": "rs_srv_rate", "style": "success"}]
    ]}
    ...
    kb["inline_keyboard"].append([{"text": "BACK", "icon_custom_emoji_id": "5267490665117275176", "callback_data": "rs_control", "style": "danger"}])
    return kb
    
def rs_manage_w_methods_keyboard():
    kb = {"inline_keyboard": []}
    ...
    kb["inline_keyboard"].append([{"text": "BACK", "callback_data": "rs_control", "style": "primary"}])
    return kb
    ...
    if fetched_numbers:
        ...
        for f_num, ctry, op in fetched_numbers:
            ...
            kb["inline_keyboard"].append([
                {"text": f"+{clean_f_num}", "icon_custom_emoji_id": "6204108584381322968", "copy_text": {"text": f"+{clean_f_num}"}, "style": "primary"}
            ])
            
        kb["inline_keyboard"].append([
            {"text": "Change Number", "icon_custom_emoji_id": "6206118633370818254", "callback_data": f"cc_{sid_short}_{c_code}", "style": "danger"},
            {"text": "OTP Group", "icon_custom_emoji_id": "6289324171507800449", "url": otp_button_link, "style": "success"}
        ])
        kb["inline_keyboard"].append([
...
def handle_message(message):
    ...
    if "chat_shared" in message:
        ...
        if user_id == ADMIN_ID and user_id in user_states:
            ...
            elif state == "waiting_rs_w_group":
                bot_settings["w_group"] = str(shared_chat_id)
                save_local_data()
                if target_msg_id: edit_message(chat_id, target_msg_id, "⚙️ <b>RS CONTROL PANEL</b>\nManage core bot configurations:", reply_markup=rs_control_keyboard())
                del user_states[user_id]
                return
    ...
    if text in ["GET NUMBER", "LIVE TRAFFIC", "REFER", "PROFILE", "LEADERBOARD", "OWNER PANEL"]:
        if user_id in user_states: del user_states[user_id]
    ...
    if text.startswith("/start"):
        ...
        profile_txt = (
            f"<tg-emoji emoji-id=\"6206185428702206246\">💰</tg-emoji> <b>YOUR PROFILE</b>\n"
            f"— — — — — — — — — —\n"
            f"<tg-emoji emoji-id=\"5352861489541714456\">👤</tg-emoji> <b>NAME :</b> {html.escape(str(first_name))}\n"
            f"— — — — — — — — — —\n"
            f"<tg-emoji emoji-id=\"5354968347094046619\">🆔</tg-emoji> <b>ID :</b> <code>{user_id}</code>\n"
            f"— — — — — — — — — —\n"
            f"<tg-emoji emoji-id=\"6190336264940559752\">💵</tg-emoji> <b>BALANCE : {user_data['balance']:.2f} BDT</b>\n"
            f"— — — — — — — — — —\n"
        ...
        send_message(chat_id, profile_txt, reply_markup=kb)
...
def handle_callback(callback_query):
    ...
    if data.startswith("admin_w_approve_") or data.startswith("admin_w_reject_"):
        ...
        masked_number = f"{acc_number[:3]}RS{acc_number[-3:]}" if len(acc_number) >= 6 else acc_number
    ...
    # ===== RS Control Panel =====
    elif data == "rs_control":
        edit_message(chat_id, message_id, "⚙️ <b>RS CONTROL PANEL</b>\nManage core bot configurations:", reply_markup=rs_control_keyboard())
        answer_callback_query(query_id)
    elif data == "rs_min_w":
        user_states[user_id] = {"state": "waiting_rs_min_w", "msg_id": message_id}
        edit_message(chat_id, message_id, "💸 <b>Enter new Minimum Withdraw Amount (e.g. 50):</b>", reply_markup=get_back_only_keyboard())
        answer_callback_query(query_id)
    elif data == "rs_ref_r":
        user_states[user_id] = {"state": "waiting_rs_ref_r", "msg_id": message_id}
        edit_message(chat_id, message_id, "💸 <b>Enter new Referral Reward Amount (e.g. 5.0):</b>", reply_markup=get_back_only_keyboard())
        answer_callback_query(query_id)
    elif data == "rs_otp_control":
        edit_message(chat_id, message_id, "💰 <b>OTP REWARD CONTROL</b>\nManage rates below:", reply_markup=rs_otp_control_keyboard())
        answer_callback_query(query_id)
    elif data == "rs_def_rate":
        user_states[user_id] = {"state": "waiting_rs_def_rate", "msg_id": message_id}
    ...
    elif data == "rs_srv_rate":
        user_states[user_id] = {"state": "waiting_rs_srv_rate", "msg_id": message_id}
        ...
        user_states[user_id] = {"state": "waiting_rs_cool", "msg_id": message_id}
        edit_message(chat_id, message_id, "⏳ <b>Enter new Cooldown time in seconds (e.g. 10):</b>", reply_markup=get_back_only_keyboard())
        answer_callback_query(query_id)
    elif data == "rs_num_req":
        user_states[user_id] = {"state": "waiting_rs_num_req", "msg_id": message_id}
    ...
    elif data == "rs_w_group":
        user_states[user_id] = {"state": "waiting_rs_w_group", "msg_id": message_id}
...
def main():
    ...
    print(f"Bot @{BOT_USERNAME} is running with Voltx, rs Control & OTP Forwarding...")
